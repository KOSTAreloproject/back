...
public class ProductService {
	...
	public Map<String, Object> selectByIdProduct(Long mNum, int currentPage) throws FindException {
		...
		if (!pList.isEmpty()) {
			...
			long totalCount = PagePList.getTotalElements();
	...
	/**
	   ...
	   public List<ZPResponseDTO> shopProductList(int start, String prodCate, String tender, String sort)
	         throws FindException {
	      String sql = "SELECT p.p_num, p.s_num, s_brand, s_name, s_hope_price, nvl(a.max_price,0) as max_price, p_end_date, s_type, z.zcount\r\n"
	      ...
	      for (Object[] objs : resultList) {
	         ZPResponseDTO dto = ZPResponseDTO.builder().pNum(Long.valueOf(String.valueOf(objs[0])))
	               .sNum(Long.valueOf(String.valueOf(objs[1])))
	               .sBrand((String) objs[2])
	               .sName((String) objs[3])
	               .sHopePrice(Integer.parseInt(String.valueOf(objs[4])))
	               .maxPrice(Integer.parseInt(String.valueOf(objs[5])))
	               .pendDate(LocalDateTime.parse(String.valueOf(objs[6]), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.S")))
	               .sType((String) objs[7]).build();
	         dtos.add(dto);
	      }
	      return dtos;
	   }
