...
public class StockService {
	...
	public Long StockAdd(StockDTO stock) throws AddException {
		...
		Stock s = Stock.builder().member(m).sizes(size).sBrand(stock.getSBrand()).sName(stock.getSName())
				.sOriginPrice(stock.getSOriginPrice()).sColor(stock.getSColor()).sType(stock.getSType())
				.sHopeDays(stock.getSHopeDays()).sellerComment(stock.getSellerComment()).sStatus(1).build();
		sr.save(s);
		return s.getSNum();
	}

	/**
	 * 관리자가 판매자가 등록한 상품에 등급과 comment를 등록한다. 판매자가 hopePrice를 등록한다. 관리자가 판매자가 등록한 상품에
	 * 등급과 comment를 등록할때 등급이 "불"을 받는다.
	 * 
	 * @param stock
	 * @throws AddException
	 */
	public void updateSetSStatus(StockDTO stock) throws AddException {
		...
		if (optM1.get().getType() == 1) { // 관리자
			...
			if (optS1.isPresent()) {
				...
				if (!stock.getSGrade().isBlank() && !stock.getManagerComment().isBlank()
						&& !stock.getSGrade().equals("불")) {
	...
	public Map<String, Object> selectById(Long mNum, int currentPage) throws FindException {
		...
		if (!sList.isEmpty()) {
			...
			for (Object[] obj : sList) {
				StockDTO dto = StockDTO.builder().sNum(Long.valueOf(String.valueOf(obj[0])))
						.sName(String.valueOf(obj[1])).sizeCategoryName(String.valueOf(obj[2]))
						.sStatus(Integer.valueOf(String.valueOf(obj[3]))).sGrade(String.valueOf(obj[4]))
						.sBrand(String.valueOf(obj[5])).build();
	...
	public List<StockDTO> detailById(Long sNum, Long mNum) throws FindException {
		...
		for (Object[] obj : sList) {
			...
			StockDTO dto = StockDTO.builder().sNum(Long.valueOf(String.valueOf(obj[0])))
					.mNum(Long.valueOf(String.valueOf(obj[1]))).sName(String.valueOf(obj[2]))
					.sType(String.valueOf(obj[3])).sizeCategoryName(String.valueOf(obj[4]))
					.sColor(String.valueOf(obj[5])).managerComment(String.valueOf(obj[6]))
					.sHopeDays(Integer.valueOf(String.valueOf(obj[7])))
					.sOriginPrice(Integer.valueOf(String.valueOf(obj[8]))).sBrand(String.valueOf(obj[9]))
					.sStatus(Integer.valueOf(String.valueOf(obj[10]))).sGrade(String.valueOf(obj[11]))
					.sHopePrice(Integer.valueOf(sHopePrice)).build();
	...
	/**
	 * 관리자 상품등록 승인요청 목록 sStatus =1 AND 관리자 상품 최종 등록 목록 sStatus =3 AND 관리자 재고반송 신청 목록
	 * sStatus =5
	 * 
	 * @return list
	 * @throws AddException
	...
	public Map<String, Object> selectBySstatus(Integer sStatus, int currentPage) throws FindException {
		...
		if (!List.isEmpty()) {
			...
			for (Object[] obj : List) {
				StockDTO dto = StockDTO.builder().sNum(Long.valueOf(String.valueOf(obj[0])))
						.sName(String.valueOf(obj[1])).sizeCategoryName(String.valueOf(obj[2]))
						.sColor(String.valueOf(obj[3])).mNum(Long.valueOf(String.valueOf(obj[4]))).build();
