...
public class StockReturnService {
   ...
   public void addStockReturn(Long mNum, Long sNum) throws AddException {
      ...
      StockReturn stockReturn = StockReturn.builder().sNum(s.getSNum()).s(s).addr(a).srTrackingInfo(String.valueOf(u)).srStartDate(LocalDate.now()).srStatus(0)
            .build();
   ...
   public  Map<String, Object> selectByIdStockReturnDetail(Long mNum,Long sNum) throws FindException {
	      ...
	      Address a = ar.findByAddrType(mNum);
	      Optional<Stock> OptS = sr.findById(mNum);
	      Stock s = OptS.get();
	     
	      Optional<StockReturn> OptS1 = srr.detailById(mNum,sNum);
	      StockReturn sr = OptS1.get();

	      sr = StockReturn.builder().sNum(sr.getSNum())
	            .srStartDate(sr.getSrStartDate())
	            .srStatus(sr.getSrStatus()).srTrackingInfo(sr.getSrTrackingInfo())
	            .addr(a)
	            .build();

	      Map<String, Object> resultMap = new HashMap<>();
	      resultMap.put("sr", sr);
	      resultMap.put("sBrand", s.getSBrand());
	      resultMap.put("sName", s.getSName());
	      resultMap.put("sizeCategoryName", s.getSizes().getSizeCategoryName());
		...
		// 재고반송 개수세기 
		 public Integer selectCntSrById(Long mNum) {
		      Integer cnt = srr.listCntById(mNum);
		      return cnt;
		   }
