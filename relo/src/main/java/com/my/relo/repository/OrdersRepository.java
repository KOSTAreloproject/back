...
public interface OrdersRepository extends CrudRepository<Orders, Long> {
	...
	/**
	 * 회원 주문 목록 select SQL 페이징버전
	 * 상품번호, 브랜드명, 상품명, 사이즈, 가격, 주문일, 배송상태
	 *  
	 * @return List<Orders> 
	 */
	@Query(value="select a.m_num, p.p_num, o.a_num, s.s_name, s.s_brand, sz.size_category_name, a.a_price, o.o_date, o.o_memo, od.d_status, o.imp_uid\r\n"
			+ "from orders o, stock s, product p, sizes sz, auction a, order_delivery od \r\n"
			+ "where a.a_num in (select aw.a_num \r\n"
			+ "from auction a, award aw \r\n"
			+ "where a.m_num= :mNum \r\n"
			+ "and a.a_num=aw.a_num)\r\n"
	...
	Page<Orders> findOrdersBymNum(@Param("mNum") Long mNum, Pageable pageable);
	
	/**
	 * 관리자가 보는 상품 구매확정 목록 select SQL 페이징
	 * 상품번호, 경매번호, 브랜드명, 상품명, 사이즈, 가격, 주문일, 배송상태, 송장번호, 구매확정일, 주소번호, 수령인, 전화번호, 우편번호, 주소, 주소상세
	 *  
	 * @return List<Orders>
	 */
