...
public interface ZzimRepository extends CrudRepository<Zzim, ZzimEmbedded> {
	 ...
	 * @return 나의 찜목록
	 */
	@Query(value = "SELECT z.m_num, z.p_num, s.s_num, s_brand, s_name, s_type, sz.size_category_name, s_color, s_hope_price, s_grade, p_end_date, nvl(a.max_price,0) as max_price\r\n"
			+ "FROM zzim z\r\n" + "JOIN product p ON z.p_num = p.p_num\r\n" + "JOIN stock s ON p.s_num = s.s_num\r\n"
			...
			+ "WHERE z.m_num = :mNum\r\n"
			+ "ORDER BY p_end_date", countQuery = "SELECT count(*) FROM zzim z WHERE z.m_num = :mNum ", nativeQuery = true)
