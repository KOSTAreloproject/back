...
public interface ProductRepository extends CrudRepository<Product, Long> {
	...
	//@Modifying    insert, update, delete
		...
		@Query(value = "select p.s_name,s.SIZE_CATEGORY_NAME,p.P_STATUS,p.p_num,p.s_brand\r\n"
				+ "from p_info p INNER JOIN sizes s on p.size_category_num = s.size_category_num\r\n"
				+ "where p.M_NUM=:mNum and (p.p_status = 6 or p.p_status = 8)"
				, countQuery = "select count(*) from stock", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM " + "(SELECT rownum rn, t.* FROM "
			+ "(SELECT a.p_num, a.m_num, m.id, a_price, a_date FROM auction a JOIN product p ON a.p_num = p.p_num "
			+ "JOIN stock s ON p.s_num = s.s_num " + "JOIN member m ON s.m_num = m.m_num "
			+ "WHERE a.p_num = :pNum ORDER BY a_price DESC " + ") t) WHERE rn BETWEEN 1 AND 20", nativeQuery = true)
