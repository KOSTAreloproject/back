...
public interface ProductRepository extends CrudRepository<Product, Long> {
	...
	@Query(value = "select p.S_NAME,s.SIZE_CATEGORY_NAME,p.P_STATUS,p.P_NUM,p.S_BRAND,p.S_NUM \r\n"
			...
			+ "where p.M_NUM = :mNum and p.p_status = 4", countQuery = "select count(*) from P_INFO where M_NUM = :mNum and p_status = 4", nativeQuery = true)
	...
	@Query(value = "select p.s_name,s.SIZE_CATEGORY_NAME,p.P_STATUS,P_END_DATE,p.s_hope_price,p.s_grade,p.s_brand,p.s_num\r\n"
			+ "from p_info p INNER JOIN sizes s on p.size_category_num = s.size_category_num\r\n"
			+ "where p.M_NUM=:mNum and p.p_num=:pNum", nativeQuery = true)
	...
	@Query(value = "select p.s_name,s.SIZE_CATEGORY_NAME,p.P_STATUS,p.p_num,p.s_brand,p.S_NUM,p.P_End_Date\r\n"
			+ "from p_info p INNER JOIN sizes s on p.size_category_num = s.size_category_num\r\n"
			+ "where p.M_NUM=:mNum and (p.p_status = 6 or p.p_status = 8 or p.p_status = 7 or p.p_status = 9)", countQuery = "select count(*) from p_info where M_NUM=:mNum and (p_status = 6 or p_status = 8 or p_status = 7 or p_status = 9)", nativeQuery = true)
	...
	@Query(value = "select p.s_name,s.SIZE_CATEGORY_NAME,p.p_status,p.p_end_date,p.p_num,p.s_brand,a.max_price,p.s_num\r\n"
			+ "from p_info p INNER JOIN a_max a on p.p_num = a.p_num  INNER JOIN sizes s on p.size_category_num = s.size_category_num\r\n"
			+ "where p.M_NUM=:mNum", nativeQuery = true)
	...
	@Query(value = "SELECT p.p_num, s.s_num, s.s_name, s.s_hope_price, nvl(a.max_price,0), p.p_end_date,s.s_type, nvl(z.zcount,0) \r\n"
			+ "FROM stock s\r\n" + "JOIN product p ON s.s_num = p.s_num \r\n"
			+ "LEFT OUTER JOIN (SELECT p_num, MAX(a_price) AS max_price FROM auction GROUP BY p_num) a ON p.p_num = a.p_num \r\n"
			+ "LEFT OUTER JOIN (SELECT p_num, COUNT(*) zcount FROM zzim GROUP BY p_num) z ON p.p_num = z.p_num \r\n"
			+ "WHERE p_status = 4 AND s.s_name Like %:keyword%\r\n"
			+ "ORDER BY p_end_date", countQuery = "SELECT count(*) FROM stock s WHERE s_name like %:keyword%", nativeQuery = true)
