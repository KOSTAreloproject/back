...
public interface StockRepository extends CrudRepository<Stock, Long> {
	...
	@Query(value = "select s.s_num,s.s_name,si.SIZE_CATEGORY_NAME ,s.s_status,s.s_grade,s.s_brand\r\n"
			...
			+ "where s.M_NUM=:mNum and (s.S_STATUS = 1 or s.S_STATUS = 2 or s.S_STATUS = 3)", countQuery = "select count(*) from stock", nativeQuery = true)
	...
	@Query(value = "select s.s_num,s.s_name,si.size_category_name ,s.s_color,s.m_num\r\n"
			+ "from stock s INNER JOIN sizes si on s.size_category_num = si.size_category_num\r\n"
			+ "where s.s_status =:sStatus and s_num not in(select s_num from stock_return) ", countQuery = "select count(*) from stock where s_num not in(select s_num from stock_return) ", nativeQuery = true)
